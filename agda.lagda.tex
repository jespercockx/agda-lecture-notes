...
on \url{https://docs.haskellstack.org/en/stable/README/}.} Other ways
to install Agda can be found at
\url{https://agda.readthedocs.io/en/latest/getting-started/installation.html}.
...
WSL\footnote{\url{https://docs.microsoft.com/en-us/windows/wsl/install-win10}}
...
from \url{https://code.visualstudio.com/}. To install an extension,
...
found at
\url{https://agda.readthedocs.io/en/v2.6.1.2/getting-started/hello-world.html}.}
...
for decimal numbers, which we can activate with the following pragma:
\begin{code}
{-# BUILTIN NATURAL Nat #-}
\end{code}
...
\url{https://github.com/agda/agda-stdlib}. In these notes, we will
...
\marginnote[-2cm]{
\begin{exercise}
Implement the function $\fun{downFrom} : (n : \Nat) \to \Vec\ \Nat\ n$
that, given a number $n$, produces the vector
$(n-1)\ \cons\ (n-2)\ \cons\ \ldots\ \cons\ 0$.
\end{exercise}
}
...
$\_\cons\_$. However, the type itself no longer has type $\ty{}$ but
instead $\Nat \to \ty{}$, indicating that it takes one additional
argument of type $\Nat$. This is reflected in the types of the
...
the colon in the definition of $\Vec$, while the argument $(n : \Nat)$
appears after it. The reason for this is that $A$ is a
\emph{parameter}, while $n$ is an \emph{index} of the datatype. The
main difference is that parameters must occur uniformly in the return
types of the constructors --- i.e.~all the constructors must return a
vector with elements of type $A$ --- while an index can be different for
each constructor.
}
...
\item The official Agda documentation.%
  \footnote{\url{https://agda.readthedocs.io/en/}}

\item The Agda standard library.%
  \footnote{\url{https://github.com/agda/agda-stdlib}}

\item Philip Wadler and Wen Kokke: \emph{Programming Language
  Foundations in Agda}\footnote{\url{https://plfa.github.io/}} (online
...
\item Aaron Stump: \emph{Verified Functional Programming in Agda}%
  \footnote{\url{https://www.morganclaypoolpublishers.com/catalog_Orig/product_info.php?cPath=24&products_id=908}}
...
\item Ulf Norell and James Chapman: \emph{Dependently Typed
  Programming in Agda}%
  \footnote{\url{http://www.cse.chalmers.se/~ulfn/papers/afp08/tutorial.pdf}}
  (tutorial in \texttt{pdf} format).

\item Ana Bove and Peter Dybjer: \emph{Dependent Types at Work}%
  \footnote{\url{http://www.cse.chalmers.se/~peterd/papers/DependentTypesAtWork.pdf}}
  (tutorial in \texttt{pdf} format).

\item Andreas Abel: \emph{An Introduction ot Dependent Types and Agda}%
  \footnote{\url{http://www2.tcs.ifi.lmu.de/~abel/DepTypes.pdf}} (lecture notes).

\item Andreas Abel: \emph{Agda Equality}%
  \footnote{\url{http://www2.tcs.ifi.lmu.de/~abel/Equality.pdf}} (lecture
  notes).


%\item Jesper Cockx: Dependent Pattern Matching and Proof-Relevant Unification
%(\url{https://jesper.sikanda.be/files/thesis-final-digital.pdf}, PhD thesis)
