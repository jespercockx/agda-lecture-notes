...
It can be difficult to figure out the types and normal forms, especially when
you are just starting to learn Agda. When you are unsure of the type or normal
form of an expression, first try to predict what it is and then use the
commands above to test your prediction.
...
\begin{tikzpicture}[node distance=0.2cm and 0.8cm, auto]
  \node (fz) {$\Fin\ 0$};
  \node (fsz) [right=of fz] {$\Fin\ 1$};
  \node (fssz) [right=of fsz] {$\Fin\ 2$};
  \node (fsssz) [right=of fssz] {$\Fin\ 3$};
  \node (fssssz) [right=of fsssz] {$\Fin\ 4$};
...
where each type other than $\Fin\ 0$ has an element $\zero$, and each type also
