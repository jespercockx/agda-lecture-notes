...
data Fin : Nat → Set where
  ...
  suc   : {n : Nat} → Fin n → Fin (suc n)
\end{code}
From this definition, we can see that \con{zero} constructs
...
to construct an element of type $\data{Fin}\ 0$: it
...
At the next step, there is only one way to construct an element
of type $\Fin\ 1$ which is the constructor \zero, since
